...
:- assert(prolog_files(util,'util.pl')).
...
%% ?- predicate_property(takesMedsAt(eleanorDougherty, [am(7), pm(2), pm(9)]),file(X)).
%% X = '/var/lib/myfrdcsa/codebases/internal/akahige/data/people/Eleanor-Dougherty/eleanor-dougherty.pl'.
...
%% F = '/var/lib/myfrdcsa/codebases/internal/akahige/data/people/Eleanor-Dougherty/eleanor-dougherty.pl', findall(C,(predicate_property(user:P,file(F)),clause(P,B,R),C = :-(P,B)),Cs),findall(Clause,(member(C,Cs),occurs:sub_term(eleanorDougherty,C)),Clauses),view(Clauses).
